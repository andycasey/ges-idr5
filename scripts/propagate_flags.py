...
""" Mark results as poor quality based on submitted flag information. """
...
# Flag *CLEARLY* erroneous problems from the WG11/IACAIP node. 
...
database.execute("""
    ...
    WHERE cname IN (
        ...
        FROM nodes AS n, results AS r, spectra AS s
        WHERE r.node_id = n.id 
          AND r.cname = s.cname 
          AND (s.ges_fld LIKE 'M12%' OR s.ges_fld LIKE 'NGC5927%') 
          AND n.wg = 11 
          AND ges_type LIKE 'AR_%'
        );
    """)
...
database.execute("""
    ...
    WHERE node_id = 2 
      AND cname IN (
        SELECT r1.cname FROM results AS r1, results AS r2, spectra AS s
        WHERE r1.cname = r2.cname 
          AND r1.node_id = 2
          AND r2.node_id = 10 
          AND s.cname = r1.cname 
          AND s.ges_type NOT LIKE '%_SD_B%'
          AND ges_type LIKE 'GE_CL%' 
          AND abs(r1.teff - r2.teff) > 1000
          AND r1.teff <> 'NaN'
          AND r2.teff <> 'NaN'
        );
    """)
...
database.execute("""
    DELETE FROM wg_recommended_results 
    WHERE cname IN ('21300431+1210561', '21300738+1210330')""")
database.execute("""
    UPDATE results 
    SET passed_quality_control = false 
    WHERE cname IN ('21300431+1210561', '21300738+1210330') AND node_id = 2""")


# Remove Vilnius for Rup134 to see if that improves clump.
database.execute("""
    UPDATE results
    SET passed_quality_control = false
    WHERE id IN (
        ...
        WHERE r.cname = s.cname
          AND r.node_id = n.id
          AND (n.id = 6 OR n.id = 11)
          AND r.passed_quality_control
          AND s.ges_fld LIKE 'Rup134%')
    """)
