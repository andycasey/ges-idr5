...
// This model allows for polynomial expressions of systematic uncertainty at
// *both* ends of parameter space.
...
transformed data {
    ...
    matrix[9, C] SCMA; // The normalised values of AMCS, but back-to-front.
    ...
    for (c in 1:C) {
        ...
        SCMA[1, c] = pow(1.0 - teff_norm, 2);
        SCMA[2, c] = pow(1.0 - logg_norm, 2);
        SCMA[3, c] = pow(1.0 - feh_norm, 2);
        SCMA[4, c] = 1.0 - teff_norm;
        SCMA[5, c] = 1.0 - logg_norm;
        SCMA[6, c] = 1.0 - feh_norm;
        SCMA[7, c] = (1.0 - teff_norm) * (1.0 - logg_norm);
        SCMA[8, c] = (1.0 - teff_norm) * (1.0 - feh_norm);
        SCMA[9, c] = (1.0 - logg_norm) * (1.0 - feh_norm);
    }
}
...
parameters {
    ...
    // Now we do the same but for the (1.0 - theta) terms.
    real sv_tb1;
    real sv_tb2;
    real sv_tb3;
    real sv_tb4;
    ...
    real<lower=pow(sv_tb1/2.0, 2)> sv_ta1;
    real<lower=pow(sv_tb2/2.0, 2)> sv_ta2;
    real<lower=pow(sv_tb3/2.0, 2)> sv_ta3;
    real<lower=pow(sv_tb4/2.0, 2)> sv_ta4;
    real<lower=pow(sv_tb5/2.0, 2)> sv_ta5;
    real<lower=pow(sv_tb6/2.0, 2)> sv_ta6;

    real<lower=pow(sv_lb1/2.0, 2)> sv_la1;
    real<lower=pow(sv_lb2/2.0, 2)> sv_la2;
    real<lower=pow(sv_lb3/2.0, 2)> sv_la3;
    real<lower=pow(sv_lb4/2.0, 2)> sv_la4;
    real<lower=pow(sv_lb5/2.0, 2)> sv_la5;
    real<lower=pow(sv_lb6/2.0, 2)> sv_la6;

    real<lower=pow(sv_fb1/2.0, 2)> sv_fa1;
    real<lower=pow(sv_fb2/2.0, 2)> sv_fa2;
    real<lower=pow(sv_fb3/2.0, 2)> sv_fa3;
    real<lower=pow(sv_fb4/2.0, 2)> sv_fa4;
    real<lower=pow(sv_fb5/2.0, 2)> sv_fa5;
    real<lower=pow(sv_fb6/2.0, 2)> sv_fa6;

    real<lower=0> sv_tc7[N]; // Cross term coefficients for (1.0 - teff) * (1.0 - logg) 
    real<lower=0> sv_tc8[N]; // Cross term coefficients for (1.0 - teff) * (1.0 - feh)
    real<lower=0> sv_tc9[N]; // Cross term coefficients for (1.0 - logg) * (1.0 - feh)
}
            ...
            for (v in 1:visits[c]) {
                ...
                for (n in 1:N) {
                    ...
                    sigma[n] = sqrt(
                        ...
                        vs_c[n] * (7.0 + relative_sigma_sys[n]));
